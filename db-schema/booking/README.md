...
| `Booking` | Vé của 1 buyer cho 1 trip. | `bookingCode` UNIQUE, buyer snapshot nullable cho dữ liệu cũ, 4 pickup/dropoff FK (exclusive), `totalAmount` immutable snapshot, 4 trip snapshot fields, `bookingGroupId`/`tripDirection` round-trip, `cancellationReason` enum, `refundOverride` |
...
- **Buyer của Booking là tài khoản tạo/thanh toán, không phải ghế Passenger đầu tiên.** Booking mới ghi `buyer_display_name`, `buyer_phone`, `buyer_email`, `buyer_avatar_url` từ Identity trong cùng luồng tạo. Bốn cột nullable để rollout tương thích với dữ liệu cũ; job `booking.buyer-snapshot-backfill` xử lý tối đa 100 Booking mỗi lượt và không gọi HTTP trong migration.
- **Xóa buyer luôn ưu tiên bảo vệ PII.** Consumer `identity.user.deleted` cập nhật idempotent mọi Booking có cùng `passenger_user_id` thành `buyer_display_name='Người dùng đã xóa'` và đặt phone/email/avatar về NULL. Identity không resolve được trong backfill cũng dùng tombstone này để tránh kẹt vĩnh viễn các hàng cũ nhất. List/detail chỉ gọi một Identity batch cho các hàng legacy còn thiếu snapshot; hàng đã có snapshot không gọi fallback.
...
- **`booking_transfers` 1 record per Passenger per substitution trip pair** (không phải 1 per Booking) — multi-passenger Booking có N rows. `uq_booking_transfers_passenger_trip_pair (passenger_id, original_trip_id, new_trip_id)` dedupes replay while allowing a later chained substitution with a different pair. Both seat-history values and seat-type audit values are nullable and never use a sentinel; `is_seat_downgrade` is computed authoritatively by Trip Service.
- **Physical transfer confirmation belongs only to `BookingTransfer`.** `confirmation_status` is exactly `PENDING_CONFIRM|ESCALATED|CONFIRMED|NOT_REQUIRED`; a 5-minute UTC sweep escalates `PENDING_CONFIRM` after 2 hours, while assigned crew may still confirm `ESCALATED` rows. Nullable `confirmed_at` and logical Identity FK `confirmed_by_user_id` record only physical confirmation and never rewrite Passenger boarding history or Ticket usage.
...
| `idx_booking_transfers_pending_confirm_transferred_at` | `transferred_at` partial | B-tree | 2-hour transfer escalation sweep |
...
- **Buyer snapshot rollout:** migration chỉ thêm bốn cột nullable snake_case và có `Down()` xóa đủ bốn cột; không gọi Identity và không chạy data backfill trong SQL. Application backfill chạy sau deploy bằng Hangfire, còn `identity.user.deleted` redaction tiếp tục bảo vệ cả snapshot đã có và hàng legacy.
