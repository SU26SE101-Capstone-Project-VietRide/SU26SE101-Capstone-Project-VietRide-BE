...
|---|---|---|
| `Notification` | In-app history per user. | `type` enum (34 values), `title`, `body`, `data` JSONB, `readAt` |
...
- **`Notification.type` enum với 34 value** — toàn bộ notification type liệt kê ở v6 Section 8 entity list. Mở rộng qua `ALTER TYPE ADD VALUE`.
