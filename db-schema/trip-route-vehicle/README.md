...
| `Trip.planned_eta_source` | Nguồn nội bộ của planned ETA. | `GOOGLE_ROUTES\|GOONG\|ROUTE_BASELINE`; API chỉ trả quality `TRAFFIC_AWARE\|ROUTE_BASED\|FALLBACK` |
...
- **Planned ETA do Trip Service tính** — `TripStop.estimated_arrival_time` và destination ETA dùng Goong Directions ordered waypoints khi có thể, cộng dwell mặc định 20 phút; fallback dùng cumulative Route metrics. Approved pre-departure Route edit và DriverSchedule `ALL_PENDING` được recompute. `Trip.planned_eta_source=GOONG|ROUTE_BASELINE`; persisted `GOOGLE_ROUTES` chỉ giữ để đọc lịch sử. Public quality map `GOOGLE_ROUTES → TRAFFIC_AWARE`, `GOONG → ROUTE_BASED`, `ROUTE_BASELINE → FALLBACK`; GPS dynamic ETA chỉ ở Redis và không ghi đè baseline.
