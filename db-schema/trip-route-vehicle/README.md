...
- **Trip owns replacement seats:** Trip parses the replacement Vehicle layout, creates TripSeats, reserves mapped seats, and emits one `trip.trip.vehicle_substituted` plus canonical `trip.trip.disrupted` in the same local transaction. Booking and Parcel remain logical consumers; there is no cross-service FK or foreign database write.
