...
- **Hangfire schema:** `hangfire.*` trong cùng DB. Jobs:
  ...
  - Invoice PDF retry (event-driven)
...
|---|---|---|
| `Payment` | Mọi giao dịch thanh toán (Booking/Parcel/TopUp/Subscription). | `referenceType`+`referenceId` polymorphic, `vnpayTxnRef` UNIQUE partial, `idempotencyKey` UNIQUE partial |
...
| `Invoice` | Subscription invoice VietRide→Operator. | `invoiceNumber` UNIQUE `VR-INV-yyyyMM-XXXXXX`, `status` enum, `pdfUrl` |
...
| `OperatorWalletTransaction` | Ledger immutable của OperatorWallet. | `type` enum CREDIT/DEBIT, `referenceType` TRIP_SETTLEMENT/ADJUSTMENT, balance snapshot |
| `OperatorTripSettlement` | Per-Trip settlement marker. | UNIQUE `(operator_id, trip_id)`, `status` enum 4-state, `eligibleAt`, `row_version` |
...
| `OperatorWalletTransaction.referenceId` (polymorphic) | `OperatorTripSettlement.id` (TRIP_SETTLEMENT) hoặc adjustment uuid (ADJUSTMENT) | by `referenceType` |
