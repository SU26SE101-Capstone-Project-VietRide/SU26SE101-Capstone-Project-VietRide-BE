...
const tripShuttleAssignedEventSchema = z
  .object({
...
const tripShuttleWarningEventSchema = z
  .object({
    eventId: z.string().uuid().optional(),
    mainTripId: z.string().uuid(),
...
const tripShuttleUnfulfilledEventSchema = z
  .object({
    eventId: z.string().uuid().optional(),
    mainTripId: z.string().uuid(),
    bookingId: z.string().uuid(),
...
const tripShuttleLifecycleEventSchema = z
  .object({
    ...
    cancellationScope: z.enum(['REQUEST', 'SHUTTLE_TRIP']).nullable().optional(),
  })
  .strict();
...
const shuttlePassengerRecipientSchema = z
  .object({
    passengerUserId: z.string().uuid(),
    bookingId: z.string().uuid().nullable(),
    pickupOrder: z.number().int().positive(),
...
const tripShuttleStartedEventSchema = z
  .object({
    eventId: z.string().uuid(),
    occurredAt: z.string(),
    shuttleTripId: z.string().uuid(),
    ...
    direction: directionSchema,
    passengers: z.array(shuttlePassengerRecipientSchema),
  })
  .strict();

export type TripShuttleStartedEvent = z.infer<typeof tripShuttleStartedEventSchema>;
export { tripShuttleStartedEventSchema as TripShuttleStartedEventSchema };

const tripShuttleReassignedEventSchema = z
  .object({
    eventId: z.string().uuid(),
    occurredAt: z.string(),
    shuttleTripId: z.string().uuid(),
    ...
    newDriver: z
      ...
      .strict(),
    oldVehicle: z.object({ id: z.string().uuid(), licensePlate: z.string().min(1) }).strict(),
    newVehicle: z.object({ id: z.string().uuid(), licensePlate: z.string().min(1) }).strict(),
