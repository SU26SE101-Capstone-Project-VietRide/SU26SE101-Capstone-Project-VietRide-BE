...
Adopt a **single response envelope `ApiResponse<T>` for every FE-facing HTTP response** across **both stacks** (.NET services + any NestJS HTTP endpoint), produced centrally (not hand-built per action), under five hard rules:
