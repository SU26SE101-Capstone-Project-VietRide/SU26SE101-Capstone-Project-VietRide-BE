# Reusable prompt — sequential execution of an approved day plan

## Usage

Replace `<DAY_N>` with the timeline day number, then paste everything under **Prompt** into a new orchestrator session.

Example: replace `<DAY_N>` with `19`. The orchestrator must discover the executable task IDs and dispatch order from `docs/handoff/day-19-plan.md`; do not manually copy them into this prompt.

## Prompt

```text
Chạy tuần tự toàn bộ executable tasks của Day <DAY_N> từ `docs/handoff/day-<DAY_N>-plan.md`, theo đúng mục `Dispatch order` và `Progress tracker` trong plan.
...
Không chạy song song, kể cả khi plan đánh dấu một số task parallel-safe. Không dùng git worktree. Không auto-run `/audit-day <DAY_N>`. Không push. Không tạo PR.
...
1. Đọc đầy đủ:
   ...
   - `docs/handoff/day-<DAY_N>-plan.md`.
...
1. Đọc lại nguyên block của task trong `day-<DAY_N>-plan.md`, gồm:
   - implement agent
   - review agent
   - skill
   - owned files
...
9. Khi task đã APPROVE và verification green:
   - Update đúng row của task trong `Progress tracker` của `day-<DAY_N>-plan.md`.
