...
| invariant flags | CRLF `.cs`, LF `.ts`; Hangfire uses UTC cron with explicit `TimeZoneInfo.Utc`, while comments document the equivalent `Asia/Ho_Chi_Minh` business time; jobs are retry-safe with conditional state updates; exactly-once notification intent via durable sent marker/outbox; RabbitMQ at-least-once consumer idempotency; expired subscription preserves read access |
