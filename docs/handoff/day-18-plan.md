...
| acceptance | items ordered by pickup-stop order (TripSnapshot.Stops OrderIndex; terminal-pickup ordering per Q5 — see CONDITIONAL); each item equals seatNumber/bookingCode/pickupStop/boardingStatus and NOTHING else; non-driver/assistant gets 403 FORBIDDEN; trip with no confirmed bookings gets empty 200 (not 404); ApiResponse envelope; at least 1 happy + 1 auth-isolation + 1 no-PII-leaked test; build/format/test green; contract section added at /v1/bookings/trips/{tripId}/manifest. CONDITIONAL (once non-gating Q5 is answered — resolve before dispatch): add a test with a booking that has a terminal pickup (pickupStationId set, pickupStopId NULL — v7 2227) asserting it sorts exactly where the Q5 decision places it (origin-terminal-first group vs separate group) |
