...
- **Q4 — RESOLVED (Asia/Ho_Chi_Minh inclusive date bounds):** `from`/`to` are `Asia/Ho_Chi_Minh` date bounds, inclusive at both ends. If both are omitted, default to today through today + 14 days. If exactly one is supplied, return 422 VALIDATION_ERROR.
...
| depends on | 18.0. Q4 resolved: `Asia/Ho_Chi_Minh` inclusive date bounds; both omitted defaults to today..today+14 days; exactly one supplied returns 422 VALIDATION_ERROR. |
...
| acceptance | returns trips where DriverUserId matches sub OR AssistantUserId matches sub within from..to using `Asia/Ho_Chi_Minh` date bounds inclusive at both ends; both omitted defaults to today..today+14 days; exactly one supplied returns 422 VALIDATION_ERROR; a different sub sees only their own; ApiResponse envelope (ADR 0004); at least 1 happy + 1 isolation test; Swagger annotation; build/format/test green; contract section added |
...
4. **RESOLVED:** from/to use `Asia/Ho_Chi_Minh` date bounds inclusive at both ends. If both are omitted, default to today..today+14 days. If exactly one is supplied, return 422 VALIDATION_ERROR.
