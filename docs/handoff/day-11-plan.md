...
- **Plan status**: APPROVED -- latest PLAN-REVIEW approved the patched plan on 2026-06-13. The PLAN-REVIEW gate covers remaining tasks only (11.2, 11.3, 11.4 and new 11.2-pre) because 11.0/11.1 are already completed/approved.
...
| source citations | technical_context_v7 lines 354/360-369 (Hangfire per-service schema, not for outbox); VietRide_API_Contract_v1.md internal lock-seats section, lines 1126-1127 (`ttlSeconds` default = 600s / `SEAT_LOCK_TTL_MINUTES * 60`); Task 11.0 appsettings/config seam in this plan (`SeatLock:TtlMinutes`, default 10, consumed by RedisSeatLockStore); db-schema/trip-route-vehicle/README.md line 12 (Hangfire jobs in this DB); AGENTS.md Hard invariants (CPM, banned deps). Do not cite the current BSOT Booking env block as Trip-owned config for this task. |
...
| 11.2 | todo | - | - | Now depends on 11.2-pre; includes DriverSchedule create+activation role/operator validation and minimal Identity internal user lookup if absent. |
