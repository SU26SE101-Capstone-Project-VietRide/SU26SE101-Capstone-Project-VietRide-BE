...
public sealed class RefreshCommandHandlerTests
{
        ...
        IClock? clock = null)
    {
        ...
        return new RefreshCommandHandler(users, refreshTokens, accessTokenSvc, refreshFactory, clock);
    }
    ...
    [Fact]
    public async Task Handle_AlreadyRevokedToken_RevokesFamilyAndThrows401()
    {
        var user = MakeActiveUser();
        var familyId = Guid.NewGuid();
        ...
        // Whole family must be revoked for reuse detection.
        await refreshTokens.Received(1).RevokeFamilyAsync(
            familyId,
