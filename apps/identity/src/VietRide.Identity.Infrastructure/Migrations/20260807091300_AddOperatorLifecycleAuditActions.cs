...
{
    /// <inheritdoc />
    public partial class AddOperatorLifecycleAuditActions : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(
                "ALTER TYPE activity_log_action ADD VALUE IF NOT EXISTS 'SUSPEND_OPERATOR';",
                suppressTransaction: true);
            migrationBuilder.Sql(
                "ALTER TYPE activity_log_action ADD VALUE IF NOT EXISTS 'REACTIVATE_OPERATOR';",
                suppressTransaction: true);
        }
            ...
            migrationBuilder.Sql("""
                ...
                BEGIN
                    IF EXISTS (
                        ...
                        WHERE action::text IN ('SUSPEND_OPERATOR', 'REACTIVATE_OPERATOR')
                    ) THEN
                        RAISE EXCEPTION 'Cannot remove operator lifecycle audit actions while activity logs use them'
                            USING ERRCODE = '55000';
                    END IF;
                END $$;

                ALTER TYPE activity_log_action RENAME TO activity_log_action_mobile_gap;
                ...
                DROP TYPE activity_log_action_mobile_gap;
                """);
        }
    }
}
