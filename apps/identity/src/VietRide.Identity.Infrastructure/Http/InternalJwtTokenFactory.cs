...
/// <summary>
/// BCL-only HS256 internal JWT signer for outbound Identity service calls
/// (currently the Notification email endpoint). Mirrors the per-service factory
/// used by Trip/Booking/Payment — the only shared piece is the
/// <see cref="IInternalJwtTokenProvider"/> contract and the
/// <c>InternalJwtDelegatingHandler</c> that consumes it (BSOT §5.3).
/// </summary>
public sealed class InternalJwtTokenFactory : IInternalJwtTokenProvider
{
    ...
    private const string DefaultCallerService = "identity";
    ...
    public InternalJwtTokenFactory(IConfiguration configuration)
    {
        var secret = configuration["InternalJwt:Secret"]
            ...
            ?? throw new InvalidOperationException("INTERNAL_JWT_SECRET must be configured for Identity internal calls.");

        if (secret.Length < 32)
        {
            throw new InvalidOperationException("INTERNAL_JWT_SECRET must be at least 32 characters long.");
        }
        ...
        _tokenLifetimeSeconds = configuration.GetValue("InternalJwt:TokenLifetimeSeconds", DefaultTokenLifetimeSeconds);
    }
