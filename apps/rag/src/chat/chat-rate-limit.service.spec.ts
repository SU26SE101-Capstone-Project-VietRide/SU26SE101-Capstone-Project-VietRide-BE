...
describe('ChatRateLimitService', () => {
  ...
  it('allows requests under the passenger limit', async () => {
    ...
    expect(client.incr).toHaveBeenCalledWith('rag:chat:rate:user:user-1');
    expect(client.expire).toHaveBeenCalledWith('rag:chat:rate:user:user-1', 3600);
  });
  ...
  it('uses operator bucket for operator-scoped callers', async () => {
    ...
    expect(client.incr).toHaveBeenCalledWith('rag:chat:rate:operator:operator-1');
  });

  it('throws 429 when limit is exceeded', async () => {
    client.incr.mockResolvedValue(21);
    const service = new ChatRateLimitService(redis, makeEnv());
