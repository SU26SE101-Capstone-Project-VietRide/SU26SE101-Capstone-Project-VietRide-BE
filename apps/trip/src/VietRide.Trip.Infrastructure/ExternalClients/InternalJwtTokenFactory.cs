...
/// <summary>
/// BCL-only HS256 internal JWT signer for outbound Trip service calls.
/// </summary>
public sealed class InternalJwtTokenFactory : IInternalJwtTokenProvider
{
    ...
    private const string DefaultCallerService = "trip";
    ...
    public InternalJwtTokenFactory(IConfiguration configuration)
    {
        var secret = configuration["InternalJwt:Secret"]
            ...
            ?? throw new InvalidOperationException("INTERNAL_JWT_SECRET must be configured for Trip internal calls.");
        ...
        _operatorId = configuration["InternalJwt:OperatorId"];
    }
    ...
    public string IssueToken(string subject, string? audience = null)
    {
        ...
        {
            payload["operatorId"] = _operatorId;
        }
