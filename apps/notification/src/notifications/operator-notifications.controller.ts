import { Body, Controller, ForbiddenException, Headers, HttpCode, Post, Req, UseGuards } from '@nestjs/common';
...
import { CreateOperatorAnnouncementSchema, type CreateOperatorAnnouncementDto } from './dto/create-operator-announcement.dto';
import { OperatorAnnouncementService, type OperatorAnnouncementResult } from './operator-announcement.service';
...
export class OperatorNotificationsController {
  ...
  async create(
    @Body(new ZodValidationPipe(CreateOperatorAnnouncementSchema)) dto: CreateOperatorAnnouncementDto,
    ...
    if (!user || !['OPERATOR_ADMIN', 'OPERATOR_STAFF'].includes(user.role) || !user.operatorId) {
      throw new ForbiddenException({ errorCode: 'FORBIDDEN', detail: 'Operator administrator or staff scope is required' });
    }
