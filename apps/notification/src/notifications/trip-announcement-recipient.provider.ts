...
import { INTERNAL_AUTH_HEADER, INTERNAL_JWT_AUDIENCE, INTERNAL_JWT_ISSUER } from './fcm-push.constants';
...
export class TripAnnouncementRecipientProvider {
  ...
  async resolveTripCrewUserIds(tripId: string, operatorId: string): Promise<string[]> {
    ...
    const response = await fetch(new URL(`/internal/v1/trips/${tripId}`, this.env.TRIP_INTERNAL_BASE_URL), {
      headers: { [INTERNAL_AUTH_HEADER]: `Bearer ${token}` },
      signal: AbortSignal.timeout(5_000),
    });
    if (response.status === 401) {
      throw new UnauthorizedException({ errorCode: 'TRIP_INTERNAL_AUTH_FAILED', detail: 'Trip rejected internal auth' });
    }
    if (response.status === 404) {
      throw new NotFoundException({ errorCode: 'TRIP_NOT_FOUND', detail: `Trip ${tripId} was not found` });
    }
    ...
    return [snapshot.driverUserId, snapshot.assistantUserId].filter((value): value is string => Boolean(value));
  }

  private async signInternalJwt(): Promise<string> {
    if (!this.env.INTERNAL_JWT_SECRET) throw new Error('INTERNAL_JWT_SECRET_REQUIRED_FOR_TRIP_LOOKUP');
    return new SignJWT({})
