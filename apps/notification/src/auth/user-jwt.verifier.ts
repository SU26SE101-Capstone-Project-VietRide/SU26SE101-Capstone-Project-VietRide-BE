...
import type { NotificationUser } from './notification-user.types';

export interface UserJwtVerifier {
  verify(token: string): Promise<NotificationUser>;
}

@Injectable()
export class JoseNotificationUserJwtVerifier implements UserJwtVerifier {
  ...
  async verify(token: string): Promise<NotificationUser> {
    ...
    const user: NotificationUser = { userId, role };
