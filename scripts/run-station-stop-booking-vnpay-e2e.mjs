...
const paymentBaseUrl = process.env.PAYMENT_BASE_URL || 'http://localhost:5004';
const postgresContainer = process.env.POSTGRES_CONTAINER || 'vietride_postgres';
const gatewayContainer = process.env.GATEWAY_CONTAINER || 'vietride_gateway';
const paymentContainer = process.env.PAYMENT_CONTAINER || 'vietride_payment';
const composeOverlay = process.env.E2E_COMPOSE_OVERLAY;
...
const vehicleTypeId = crypto.randomUUID();
...
function runSql(database, sql, capture = false) {
  ...
  if (result.error || result.status !== 0)
    fail(`psql failed for ${database} (status=${result.status ?? 1})`);
  return capture ? result.stdout.trim() : undefined;
}

function scalar(database, sql) {
  return (
    runSql(database, `${sql.replace(/;?\s*$/, '')};`, true)
      .split(/\r?\n/)
      .filter(Boolean)
      .at(-1) ?? ''
  );
...
async function api(method, url, token, body, expectedStatus, idempotency = false) {
  ...
  if (response.status !== expectedStatus) {
    fail(
      `${method} ${url}: expected HTTP ${expectedStatus}, got ${response.status}: ${text.slice(0, 500)}`,
    );
  }
  return json;
}
...
async function restartPayment(tmnCode = '', hashSecret = '') {
  const internalJwt = containerEnv(gatewayContainer).INTERNAL_JWT_SECRET;
  if (!internalJwt || internalJwt.length < 32)
    fail('Running Gateway does not expose a valid INTERNAL_JWT_SECRET.');
  const env = {
    ...process.env,
  ...
  const compose = [
    ...
    ...(composeOverlay ? ['-f', composeOverlay] : []),
    '--profile',
    'app',
  ];
  const result = spawnSync(
    'docker',
    [...compose, 'up', '-d', '--force-recreate', '--no-deps', 'payment'],
  ...
  await waitFor(
    ...
    async () => {
      try {
        return (await fetch(`${paymentBaseUrl}/health`)).ok;
      } catch {
        return false;
      }
    },
...
function sortedVnPayQuery(values) {
  const sorted = Object.entries(values)
    .filter(
      ([key, value]) =>
        key.toLowerCase() !== 'vnp_securehash' &&
        key.toLowerCase() !== 'vnp_securehashtype' &&
        value !== '',
    )
...
async function sendIpn(values, expectedRspCode) {
  ...
  const response = await fetch(
    `${gateway}/v1/payments/vnpay-ipn?${new URLSearchParams(parameters)}`,
  );
  const body = await response.json();
  if (response.status !== 200 || body.RspCode !== expectedRspCode) {
    fail(
      `VNPay IPN expected HTTP 200/RspCode ${expectedRspCode}, got ${response.status}/${JSON.stringify(body)}`,
    );
  }
  return body;
}
...
function seedIdentity() {
  runSql(
    ...
    values
      ('${subscriptionPlanId}','${tag} Plan','Isolated booking E2E plan',0,0,20,20,20,20,20,2000,true,true,true,true);
    ...
    values
      ('${operatorId}','${tag} Operator','${tag.slice(-16)}-BRN','${tag.slice(-16)}-TAX','operator-${tag}@example.test','+84908880009','APPROVED',now(),true),
      ('${otherOperatorId}','${tag} Other Operator','${tag.slice(-14)}-OBRN','${tag.slice(-14)}-OTAX','other-${tag}@example.test','+84908880007','APPROVED',now(),true);
    insert into vietride_identity.users
      (id,email,phone,password_hash,display_name,role,status,operator_id)
    values
      ('${operatorAdminId}','operator-admin-${tag}@example.test','+84908880019',null,'${tag} Operator Admin','OPERATOR_ADMIN','ACTIVE','${operatorId}'),
      ('${otherOperatorAdminId}','other-admin-${tag}@example.test','+84908880017',null,'${tag} Other Admin','OPERATOR_ADMIN','ACTIVE','${otherOperatorId}'),
      ('${systemAdminId}','system-admin-${tag}@example.test','+84908880016',null,'${tag} System Admin','SYSTEM_ADMIN','ACTIVE',null),
      ('${driverUserId}','driver-${tag}@example.test','+84908880015',null,'${tag} Driver','DRIVER','ACTIVE','${operatorId}');
    ...
    values
      ('${subscriptionId}','${operatorId}','${subscriptionPlanId}','ACTIVE',now(),now()+interval '30 days',0,0,0,now());
    `
        : ''
    }
...
function seedTripBase() {
  if (!ownsBaseFixtures) return;
  runSql(
    'vietride_trip',
    `
    ...
    values
      ('${vehicleTypeId}','E2E_${driverUserId.replaceAll('-', '').slice(0, 12).toUpperCase()}','${tag} Vehicle Type',12,false);
    ...
    values
      ('${vehicleId}','${operatorId}','${vehicleTypeId}','E2E${driverUserId.replaceAll('-', '').slice(0, 8).toUpperCase()}',
       '{"version":1,"totalSeats":12,"rows":6,"cols":2,"decks":1,"aisles":[],"seats":[]}'::jsonb,
       12,500,10,'ACTIVE',true);
  `,
  );
}
...
function seedTrips(originStationId, destinationStationId, stopId) {
  ...
  runSql(
    ...
    values
      ('${stopTripId}','${operatorId}','${outboundRouteId}','${selectedVehicleId}','${selectedDriverUserId}',now()+interval '10 days',now()+interval '10 days 6 hours','SCHEDULED','MANUAL',125000),
      ('${paymentTripId}','${operatorId}','${outboundRouteId}','${selectedVehicleId}','${selectedDriverUserId}',now()+interval '11 days',now()+interval '11 days 6 hours','SCHEDULED','MANUAL',125000),
      ('${roundOutboundTripId}','${operatorId}','${outboundRouteId}','${selectedVehicleId}','${selectedDriverUserId}',now()+interval '12 days',now()+interval '12 days 6 hours','SCHEDULED','MANUAL',125000),
      ('${roundReturnTripId}','${operatorId}','${returnRouteId}','${selectedVehicleId}','${selectedDriverUserId}',now()+interval '14 days',now()+interval '14 days 6 hours','SCHEDULED','MANUAL',125000);
    ...
    insert into vietride_trip.trip_stop_fares (trip_id,stop_id,fare_from_this_stop,source)
    values ('${stopTripId}','${stopId}',99000,'MANUAL_OVERRIDE');
    commit;
  `,
  );
}
...
async function testStationAndStopApis() {
  ...
  const patched = await api(
    'PATCH',
    `/v1/operator/stations/${origin}`,
    operatorToken,
    {
      displayNameOverride: `${tag} operator override`,
      counterLocation: 'Counter Z',
      contactPhone: '+84909999999',
      instructions: 'Updated E2E',
    },
    200,
    true,
  );
  if (patched?.data?.displayNameOverride !== `${tag} operator override`)
    fail('Operator station PATCH projection mismatch.');
  await api(
    'PATCH',
    `/v1/operator/stations/${origin}`,
    otherOperatorToken,
    { counterLocation: 'forbidden' },
    404,
    true,
  );
  ...
  if (
    scalar(
      'vietride_trip',
      `select is_active from vietride_trip.operator_stations where operator_id='${operatorId}' and station_id='${origin}'`,
    ) !== 'f'
  )
    fail('Operator station DELETE did not deactivate mapping.');
  const relink = await api(
    ...
    {
      stationId: origin,
      counterLocation: 'Relinked counter',
      contactPhone: '+84907777777',
      instructions: 'Relinked',
    },
    200,
  );
  ...
  const adminPatch = await api(
    'PATCH',
    `/v1/admin/stations/${origin}`,
    systemAdminToken,
    {
      name: `${tag} Origin Canonical`,
      city: 'Thu Duc',
      province: 'Ho Chi Minh',
      addressStreet: `${tag} canonical address`,
    },
    200,
    true,
  );
  if (
    adminPatch?.data?.name !== `${tag} Origin Canonical` ||
    adminPatch.data.slug === before.data.slug
  )
    fail('Admin Station PATCH did not update canonical name/slug.');
  const stationList = await api(
    'GET',
    `/v1/admin/stations?search=${encodeURIComponent(tag)}&page=1&pageSize=20`,
    systemAdminToken,
    undefined,
    200,
  );
  if (!stationList?.data?.items?.some((item) => item.id === origin))
    fail('Admin Station list did not return fixture.');
  const deleted = await api(
    'DELETE',
    `/v1/admin/stations/${disposable}`,
    systemAdminToken,
    undefined,
    200,
    true,
  ...
  if (
    scalar(
      'vietride_trip',
      `select count(*) from vietride_trip.operator_stations where station_id='${disposable}' and is_active`,
    ) !== '0'
  )
    fail('Admin Station DELETE did not deactivate mappings.');
  ...
  const stopPatch = await api(
    'PATCH',
    `/v1/operator/stops/${affected}`,
    operatorToken,
    {
      name: `${tag} Affected Stop Updated`,
      latitude: 21.1002,
      longitude: 101.1002,
      description: 'updated',
      address: 'updated address',
      googlePlaceId: `${tag}-updated`,
    },
    200,
    true,
  );
  if (stopPatch?.data?.name !== `${tag} Affected Stop Updated`)
    fail('Operator Stop PATCH projection mismatch.');
  await api(
    'PATCH',
    `/v1/operator/stops/${affected}`,
    otherOperatorToken,
  ...
  const adminList = await api(
    'GET',
    `/v1/admin/stops?operatorId=${operatorId}&search=${encodeURIComponent(tag)}&page=1&pageSize=20`,
    systemAdminToken,
    undefined,
    200,
  );
  if (!adminList?.data?.items?.some((item) => item.id === affected))
    fail('Admin Stop filtered list did not return fixture.');
  const adminPatched = await api(
    'PATCH',
    `/v1/admin/stops/${adminStop}`,
    systemAdminToken,
    { name: `${tag} Admin Stop Updated`, isActive: true },
    200,
    true,
  );
  if (adminPatched?.data?.name !== `${tag} Admin Stop Updated`)
    fail('Admin Stop PATCH projection mismatch.');
  await api(
    'DELETE',
    `/v1/admin/stops/${adminStop}?replacedByStopId=${replacement}`,
    systemAdminToken,
    undefined,
    200,
    true,
...
async function testTripAndBookingContract(origin, destination, affected) {
  ...
  if (JSON.stringify(Object.keys(stop).sort()) !== JSON.stringify(expectedKeys))
    fail(`Trip Stop keys mismatch: ${Object.keys(stop).sort()}`);
  if (
    stop.name !== `${tag} Affected Stop Updated` ||
    stop.address !== 'updated address' ||
    stop.fareFromThisStop !== 99000 ||
    stop.effectiveFare !== 99000 ||
    stop.distanceFromOriginKm !== 42.5
  )
    fail(`Trip Stop values mismatch: ${JSON.stringify(stop)}`);
  ...
  if (duplicate?.error?.code !== 'VALIDATION_ERROR')
    fail(`Duplicate seats returned ${duplicate?.error?.code}.`);
  ...
  if (
    missing?.error?.code !== 'PAYMENT_VNPAY_ERROR' ||
    !missing?.error?.message?.includes('not configured')
  )
    fail(`Missing VNPay config mapping mismatch: ${JSON.stringify(missing)}`);
  if (
    scalar(
      'vietride_booking',
    ...
    fail('Missing VNPay config left a Booking row behind.');
  if (
    scalar(
      'vietride_trip',
      `select status from vietride_trip.trip_seats where trip_id='${stopTripId}' and seat_number='MISS1'`,
    ) !== 'AVAILABLE'
  )
    fail('Missing VNPay config did not release seat.');
  pass(
    'VNPay development missing-config error seam',
    'HTTP 502 PAYMENT_VNPAY_ERROR; no orphan booking/lock',
  );
  ...
  if (!walletBookingId || wallet.data.status !== 'CONFIRMED' || wallet.data.tickets?.length !== 1)
    fail(`PII-free Wallet booking failed: ${JSON.stringify(wallet)}`);
...
async function testStopDisable(affected, replacement) {
  const result = await api(
    'DELETE',
    `/v1/operator/stops/${affected}?replacedByStopId=${replacement}`,
    operatorToken,
    undefined,
    200,
    true,
  ...
  const outboxCount = scalar(
    'vietride_trip',
    `select count(*) from vietride_trip.outbox_events where event_type='trip.stop.disabled' and payload::text like '%${affected}%'`,
  );
  if (outboxCount !== '1') fail(`Expected one trip.stop.disabled outbox row, got ${outboxCount}.`);
  await waitFor(
    'STOP_DISABLED pending action',
    async () =>
      scalar(
        'vietride_booking',
        `select count(*) from vietride_booking.booking_pending_actions a join vietride_booking.bookings b on b.id=a.booking_id where b.passenger_user_id='${passengerId}' and a.reason='STOP_DISABLED' and a.resolved_at is null`,
      ) === '1',
  );
  const deadlineValid = scalar(
    'vietride_booking',
    `select (a.deadline <= least(now()+interval '24 hours 1 minute', b.trip_snapshot_departure-interval '2 hours'))::text from vietride_booking.booking_pending_actions a join vietride_booking.bookings b on b.id=a.booking_id where b.passenger_user_id='${passengerId}' and a.reason='STOP_DISABLED' limit 1`,
  );
  if (deadlineValid !== 'true')
    fail('STOP_DISABLED deadline is outside min(now+24h, departure-2h).');
  await waitFor(
    'STOP_DISABLED notification',
    async () =>
      scalar(
        'vietride_notification',
        `select count(*) from vietride_notification.notifications where user_id='${passengerId}' and type='STOP_DISABLED'`,
      ) === '1',
  );
  await new Promise((resolve) => setTimeout(resolve, 1500));
  const pendingCount = scalar(
    'vietride_booking',
    `select count(*) from vietride_booking.booking_pending_actions a join vietride_booking.bookings b on b.id=a.booking_id where b.passenger_user_id='${passengerId}' and a.reason='STOP_DISABLED'`,
  );
  const notificationCount = scalar(
    'vietride_notification',
    `select count(*) from vietride_notification.notifications where user_id='${passengerId}' and type='STOP_DISABLED'`,
  );
  if (pendingCount !== '1' || notificationCount !== '1')
    fail(
      `Stop disable was not idempotent: pending=${pendingCount}, notifications=${notificationCount}.`,
    );
  pass(
...
async function createVnPayBooking(tripId, origin, destination, seatNumber, verifyReplay = false) {
  const request = {
    tripId,
    pickup: { stationId: origin },
    dropoff: { stationId: destination },
  ...
  if (verifyReplay) {
    const replay = await api('POST', '/v1/bookings', passengerToken, request, 201, idempotencyKey);
    if (
      replay?.data?.bookingId !== result.data.bookingId ||
      replay?.data?.paymentId !== result.data.paymentId ||
      JSON.stringify(replay.data) !== JSON.stringify(result.data)
    )
      fail(`One-way booking replay drifted: ${JSON.stringify({ result, replay })}`);
    const persisted = scalar(
      'vietride_booking',
      `select count(*) from vietride_booking.bookings where id='${result.data.bookingId}'`,
    );
    const payments = scalar(
      'vietride_payment',
      `select count(*) from vietride_payment.payments where reference_type='BOOKING' and reference_id='${result.data.bookingId}'`,
    );
    if (persisted !== '1' || payments !== '1')
      fail(`One-way replay duplicated state: bookings=${persisted}, payments=${payments}`);
    pass(
      'Booking -> Payment -> Trip same-key replay',
      `booking=${result.data.bookingId}; payment=${result.data.paymentId}`,
    );
  }
...
function validateRedirect(data, expectedAmount, verifyExpiry = true) {
  ...
  if (url.origin + url.pathname !== 'https://sandbox.vnpayment.vn/paymentv2/vpcpay.html')
    fail(`Unexpected VNPay base URL: ${url}`);
  if (
...
async function testVnPay(origin, destination) {
  ...
  if (invalidResponse.status !== 200 || invalid.RspCode !== '97')
    fail(`Invalid signature mapping mismatch: ${JSON.stringify(invalid)}`);
  if (paymentRow(oneWay.paymentId)[2] !== 'PENDING_REDIRECT')
  ...
  if (
    scalar(
      'vietride_trip',
      `select status from vietride_trip.trip_seats where trip_id='${paymentTripId}' and seat_number='ONE1'`,
    ) !== 'BOOKED'
  )
  ...
  await sendIpn(
    {
      vnp_TmnCode: 'E2ETMN',
      vnp_TxnRef: amountTxnRef,
      vnp_Amount: String(Number(correctAmount) * 100 + 100),
      vnp_ResponseCode: '00',
      vnp_TransactionStatus: '00',
    },
    '04',
  ...
  if (
    scalar(
      'vietride_trip',
      `select status from vietride_trip.trip_seats where trip_id='${paymentTripId}' and seat_number='AMOUNT1'`,
    ) !== 'AVAILABLE'
  )
    fail('Amount mismatch did not release seat.');
  pass('VNPay amount mismatch', 'HTTP 200/RspCode 04; booking expired; seat released');

  const groupRequest = {
    outbound: {
      tripId: roundOutboundTripId,
      pickup: { stationId: origin },
      dropoff: { stationId: destination },
      seats: [{ seatNumber: 'GROUP1' }],
    },
    return: {
      tripId: roundReturnTripId,
      pickup: { stationId: destination },
      dropoff: { stationId: origin },
      seats: [{ seatNumber: 'GROUP1' }],
    },
    paymentMethod: 'VNPAY',
  };
  ...
  if (JSON.stringify(groupReplay?.data) !== JSON.stringify(groupData))
    fail(`Round-trip replay drifted: ${JSON.stringify({ group, groupReplay })}`);
  const roundTripBookings = scalar(
    'vietride_booking',
    `select count(*) from vietride_booking.bookings where id in ('${groupData.outbound.bookingId}','${groupData.return.bookingId}')`,
  );
  const roundTripPayments = scalar(
    'vietride_payment',
    `select count(*) from vietride_payment.payments where id='${groupData.paymentId}'`,
  );
  if (roundTripBookings !== '2' || roundTripPayments !== '1')
    fail(
      `Round-trip replay duplicated state: bookings=${roundTripBookings}, payments=${roundTripPayments}`,
    );
  pass(
    'Round-trip Booking -> Payment -> Trip same-key replay',
    `group=${groupData.bookingGroupId}; payment=${groupData.paymentId}`,
  );
  ...
  await sendIpn(
    {
      vnp_TmnCode: 'E2ETMN',
      vnp_TxnRef: groupTxnRef,
      vnp_Amount: String(Number(groupAmount) * 100),
      vnp_ResponseCode: '00',
      vnp_TransactionStatus: '00',
    },
    '00',
  ...
  await Promise.all([
    pollBookingStatus(groupData.outbound.bookingId, 'CONFIRMED'),
    pollBookingStatus(groupData.return.bookingId, 'CONFIRMED'),
  ]);
  const bookedGroupSeats = scalar(
    'vietride_trip',
    `select count(*) from vietride_trip.trip_seats where trip_id in ('${roundOutboundTripId}','${roundReturnTripId}') and seat_number='GROUP1' and status='BOOKED'`,
  );
  ...
  await sendIpn(
    {
      vnp_TmnCode: 'E2ETMN',
      vnp_TxnRef: failedTxnRef,
      vnp_Amount: String(Number(failedAmount) * 100),
      vnp_ResponseCode: '24',
  ...
  await Promise.all([
    pollBookingStatus(failedGroup.data.outbound.bookingId, 'EXPIRED'),
    pollBookingStatus(failedGroup.data.return.bookingId, 'EXPIRED'),
  ]);
  const releasedGroupSeats = scalar(
    'vietride_trip',
    `select count(*) from vietride_trip.trip_seats where trip_id in ('${roundOutboundTripId}','${roundReturnTripId}') and seat_number='GROUPFAIL1' and status='AVAILABLE'`,
  );
  if (releasedGroupSeats !== '2')
    fail(`Round-trip failure released ${releasedGroupSeats}/2 seats.`);
  ...
  runSql(
    'vietride_payment',
    `update vietride_payment.payments set created_at=now()-interval '10 minutes 2 seconds' where id='${timeout.paymentId}';`,
  );
  await waitFor(
    'VNPay timeout payment',
    async () => paymentRow(timeout.paymentId)[2] === 'EXPIRED',
    180,
    500,
  );
  ...
  if (
    scalar(
      'vietride_trip',
      `select status from vietride_trip.trip_seats where trip_id='${paymentTripId}' and seat_number='TIMEOUT1'`,
    ) !== 'AVAILABLE'
  )
    fail('Timed-out payment did not release seat.');
...
function cleanup() {
  ...
  const refs = paymentReferenceIds.length
    ? paymentReferenceIds.map((id) => `'${id}'`).join(',')
    : "'00000000-0000-0000-0000-000000000000'";
...
let runError;
try {
  platformBalanceBefore = scalar(
    'vietride_payment',
    'select coalesce((select balance from vietride_payment.platform_wallets limit 1),0)',
  );
  seedIdentity();
  seedTripBase();
  [operatorToken, otherOperatorToken, systemAdminToken, passengerToken] = await Promise.all([
    ...
    issueToken({
      subject: otherOperatorAdminId,
      role: 'OPERATOR_ADMIN',
      email: 'day7-approved-admin@example.test',
      operator: otherOperatorId,
    }),
    issueToken({
      subject: systemAdminId,
      role: 'SYSTEM_ADMIN',
      email: 'e2e-full-system-admin@vietride.local',
    }),
    issueToken({ subject: passengerId, role: 'PASSENGER', email: `${tag}@example.test` }),
  ]);
  const fixture = await testStationAndStopApis();
  if (idempotencyFocused) {
    seedTrips(fixture.origin, fixture.destination, fixture.affected);
  } else {
  ...
  pass(idempotencyFocused ? 'IDEMPOTENCY FOCUSED E2E' : 'FULL E2E', tag);
} catch (error) {
  runError = error;
