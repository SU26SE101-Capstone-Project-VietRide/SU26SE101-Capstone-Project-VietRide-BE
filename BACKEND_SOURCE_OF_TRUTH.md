...
| `POST /internal/v1/trips/summaries/batch` | Booking, Parcel | Read-only `{ tripIds }`, 1..100 distinct UUIDs; one Trip query returns route/station/vehicle/crew/timing summaries; missing IDs are omitted. Booking uses one distinct-ID batch per non-empty passenger-history page to project the current `vehicle { licensePlate }`; enrichment is fail-open per item. |
