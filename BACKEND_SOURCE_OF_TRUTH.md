...
| | `REPLACEMENT_VEHICLE_INSUFFICIENT_SEATS` | 409 | Replacement usable-seat count is below eligible substitution passengers and the Operator has not acknowledged the shortage; ADR 0004 `error.fields[]` carries `usableSeats`, `passengersToTransfer`, and `missingSeats` as string messages |
...
| `POST /internal/v1/trips/{tripId}/cargo/reserve` · `remeasure` · `load` · `release` | Parcel | Idempotent single-Trip cargo-ledger mutation and counter update. Capacity conflicts use `TRIP_CARGO_CAPACITY_EXCEEDED`; invalid ledger transitions use `TRIP_CARGO_STATE_INVALID` and must not be persisted by Parcel as a capacity action. |
...
| `GET /internal/v1/bookings/history?userId=&status=&from=&to=&page=&pageSize=` | Parcel | Owner-scoped Booking history for the passenger facade; pages Booking aggregates, includes Ticket summaries and always-serialized nullable `vehicle { licensePlate, vehicleType: { code, displayName } | null }`, uses `[from,to)` over `created_at`, and orders `created_at DESC, id DESC`. Booking trims strings and only creates `vehicleType` when both fields are valid; an older Trip response without the field preserves the plate with `vehicleType=null`. Trip enrichment failure leaves `vehicle=null` without failing history. |
...
| `identity.operator.rejected` | Identity | Notification (email) | Exact `{ eventId, occurredAt, operatorId, companyName, contactEmail, reason }`; Identity snapshots the registration email and rejection reason in the same transaction as the rejected Operator, cancelled pending subscription, and activity log. Notification sends email only; no in-app/push notification is created. |
| `identity.operator.suspended` | Identity | Trip, Booking | `{ operatorId, suspendedAt }` |
| `identity.subscription_custom_request.submitted` | Identity | Notification (System Admin web) | Exact `{ eventId, occurredAt, requestId, operatorId, operatorName }`; request, activity log và Outbox commit nguyên tử; `eventId == OutboxEvent.Id == RabbitMQ MessageId`. Notification fan-out tới toàn bộ `SYSTEM_ADMIN` đang hoạt động, lưu inbox và phát Socket.IO nhưng không enqueue FCM/email. |
| `identity.subscription_custom_request.approved` | Identity | Notification (Operator Admin web) | Exact `{ eventId, occurredAt, requestId, operatorId, approvedPlanId, planName }`; custom plan, request transition, activity log và Outbox commit nguyên tử. Notification chỉ fan-out tới `OPERATOR_ADMIN` đang hoạt động của đúng `operatorId`, lưu inbox và phát Socket.IO nhưng không enqueue FCM/email. |
| `identity.subscription_custom_request.rejected` | Identity | Notification (Operator Admin web) | Exact `{ eventId, occurredAt, requestId, operatorId, rejectionReason }`; request transition, activity log và Outbox commit nguyên tử. Notification chỉ fan-out tới `OPERATOR_ADMIN` đang hoạt động của đúng `operatorId`, lưu inbox và phát Socket.IO nhưng không enqueue FCM/email. |
...
| `booking.booking.transferred` | Booking | Notification, Tracking | Exact `{eventId,occurredAt,sourceSubstitutionEventId,bookingId,recipientUserId,operatorId,oldTripId,newTripId,newVehicleId,newVehiclePlateNumber,newTripDepartureDateTime,notifyPassengers,transfers:[{passengerId,originalSeatNumber,newSeatNumber,confirmationStatus,originalBoardingStatus?}]}`; additive `originalBoardingStatus` is optional for legacy payload compatibility. Exactly one fact per eligible Booking per substitution. Notification always informs the owner when any transfer lacks a seat or was originally `PENDING`; otherwise it follows `notifyPassengers`. Tracking emits `booking:updated` with `reason=BOOKING_TRANSFERRED` into both old/new Trip crew rooms. |
| `booking.booking.seat_shortage_detected` | Booking | Notification (operator) | Exact `{eventId,occurredAt,sourceSubstitutionEventId,bookingId,bookingCode,operatorId,oldTripId,newTripId,affectedPassengerCount,originalSeatNumbers}`; one fact per affected Booking per substitution when at least one transfer has `newSeatNumber=null`. Notification fans out to active Operator Admins. |
| `booking.booking.transfer_escalated` | Booking | Notification (operator) | Exact `{eventId,occurredAt,bookingId,bookingCode,operatorId,oldTripId,newTripId,transferIds,pendingConfirmationCount,oldestTransferredAt}`; one fact per Booking + replacement Trip group only when the recurring job CAS-changes one or more rows from `PENDING_CONFIRM` to `ESCALATED`. Notification fans out to active Operator Admins. |
...
| `trip.trip.vehicle_substituted` | Trip | Booking, Parcel (Day 35) | Exact `{eventId,occurredAt,substitutionId,disruptedAt,operatorId,oldTripId,oldTripStatus,oldVehicleId,newTripId,newTripStatus,newVehicleId,newVehiclePlateNumber,newTripDepartureDateTime,actorUserId,reason,notifyPassengers,mappings:[{bookingId,passengerId,originalSeatNumber,newSeatNumber,originalBoardingStatus,originalSeatType?,newSeatType?,isSeatDowngrade?}]}`; exactly one fact per substitution; `occurredAt = disruptedAt`; `substitutionId = eventId`; `oldTripStatus=DISRUPTED`; `newTripStatus=BOARDING`; both seat numbers and types are nullable; additive type/downgrade fields are optional for legacy payload compatibility; downgrade rank is `STANDARD < SLEEPER_UPPER < SLEEPER_LOWER < VIP`; `originalBoardingStatus=BOARDED\|PENDING`; `payload.eventId == Outbox row id == RabbitMQ MessageId`. |
...
- Vehicle substitution does not change eligible Booking status: `CONFIRMED|PARTIAL_NO_SHOW` is
  ...
  transfer, or Outbox effect. Transfer evidence stores nullable original/new seat type plus
  `isSeatDowngrade`; substitution never creates `PENDING_SEAT_ASSIGNMENT`. Any no-seat mapping emits
  one Booking-scoped operator shortage fact. A five-minute UTC job changes groups older than two
  hours from `PENDING_CONFIRM` to `ESCALATED` in batches of at most 200 and emits only for rows that
  changed; `ESCALATED -> CONFIRMED` remains valid and confirmed replay remains idempotent.
...
- `netAmount` luôn recompute từ immutable operator ledger tại settle time. Nếu `netAmount <= 0`, row chuyển `CANCELLED` và không tạo wallet movement/event. Marker lưu nullable `cancel_reason`: completion có `source=VEHICLE_SUBSTITUTION` và net `0` dùng `VEHICLE_SUBSTITUTION_REVENUE_RETAINED_ON_ORIGINAL_TRIP`; mọi marker non-positive khác dùng `NON_POSITIVE_NET_ENTITLEMENT`. Doanh thu vẫn thuộc Trip cũ nên tổng net không bị nhân đôi.
...
| `BookingTransferEscalationJob` | Recurring | Every 5 phút UTC | Select at most 200 `(bookingId,newTripId)` groups whose `PENDING_CONFIRM` transfers are older than 2 hours, lock/recheck and CAS them to `ESCALATED`; emit one `booking.booking.transfer_escalated` only when at least one row changes. Late `ESCALATED -> CONFIRMED` remains allowed. |
