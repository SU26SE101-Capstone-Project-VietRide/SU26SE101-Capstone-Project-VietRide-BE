...
public sealed class Day24OutboxRestartIdentityTests : IAsyncLifetime
{
    public static TheoryData<string> Day24RoutingKeys => new()
    {
        "trip.stop.disabled",
    ...
    public Day24OutboxRestartIdentityTests(OutboxStoreFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    [Theory]
    [MemberData(nameof(Day24RoutingKeys))]
    public async Task Restart_RedeliversSamePersistedIdentity(string routingKey)
    {
        ...
        await using (var persistedContext = _fixture.CreateContext())
        {
            ...
            enqueuedRow.Status.Should().Be(OutboxEventStatus.PENDING);
            enqueuedRow.PublishedAt.Should().BeNull();
