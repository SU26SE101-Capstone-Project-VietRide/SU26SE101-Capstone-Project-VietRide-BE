...
public sealed class Day29CargoNearFullOutboxRestartTests : IAsyncLifetime
{
    private const string RoutingKey = "trip.cargo.threshold_crossed";
    private readonly OutboxStoreFixture fixture;

    public Day29CargoNearFullOutboxRestartTests(OutboxStoreFixture fixture)
    {
    ...
    [Fact]
    public async Task PendingCargoThresholdCrossedRow_SurvivesFailureAndIsDeliveredAfterWorkerRestart()
    {
        ...
        await unavailablePublisher.Received(1).PublishRawAsync(RoutingKey, eventId, persistedPayload, Arg.Any<CancellationToken>());
        await restartedPublisher.Received(1).PublishRawAsync(RoutingKey, eventId, persistedPayload, Arg.Any<CancellationToken>());
